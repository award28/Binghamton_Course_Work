...
> composeHOF lst num = foldr1 (.) lst num 
...
> sum' (x:xs) = x + sum' xs
> harmonic = 1:[1/i | i <- [2..]]
> sumHarmonic i = sum' (take i harmonic)
...
> truth x y = x == True && x == y

> allTrue lst = foldr truth True lst

    ...> allTrue [True, True, 5 == 4 + 1, False, True]
        False
    ...> allTrue [True, True, 5 == 4 + 1, 5 > 4]
        True

Problem 7.
Using a HOF (map, fold and/or filter ) define flattenT that returns a list of
...
> flattenT lst = concat (map open lst)

> rev' (x:[]) = [x]
> rev' (x:xs) = (rev' xs) ++ [x]

> flattenR lst = rev' (flattenT lst)
...
> squared x = x*x
> sumSqr n = foldr (+) 0 ( map squared (digits n))
