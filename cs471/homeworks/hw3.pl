...
is_increasing([]) .
is_increasing([_X]) .
is_increasing([H1,H2|T]):- (H2>=H1), is_increasing([H2|T]).
...
/* Problem 4 Answer: */
element_at(X,[X|_T],0).
element_at(X,[_H|T],N):- N1 is N - 1, element_at(X,T,N1).
...
/* Problem 5 Answer: */
insert_at(X, T, 0, [X|T]).
insert_at(X, [H|T], N, [H|T1]):- N1 is N-1, insert_at(X, T, N1, T1).
...
/* Problem 6 Answer: */
zip([],[],[]).
zip([XH|XT], [YH|YT], Zs):- zip(XT, YT, Z), Zs = [(XH, YH) | Z] .
...
zip2([], _Y, []).
zip2([],[],[]).
zip2([XH|XT], [YH|YT], Zs):- zip2(XT, YT, Z), Zs = [(XH, YH) | Z] .
...
merge([],_B,_M).
merge(_A,[],_M).
merge([AH|AT],[BH|BT],[AH|MT]):- (AH>=BH), merge(AT, [BH|BT], MT).
merge([AH|AT],[BH|BT],[BH|MT]):- (BH>=AH), merge([AH|AT], BT, MT).
...
greater_than(succ(succ(X)), succ(Y)):- greater_than(succ(X), Y).
...
/* Problem 10 Answer: */
subtract(Y,0,Y).
subtract(succ(X),succ(Z), Y):- subtract(X,Z,Y).
